...
export function Top100View({ category }: { category: number }): JSX.Element {
  const { movies, loading, error, pollNow } = useTopMovies(category)
    ...
    <section className="view">
      ...
      {!loading && movies.length === 0 ? (
        <div className="empty">
          ...
          <p>If this persists, check Settings and add your TMDB API key, then click Refresh now.</p>
        </div>
      ) : null}

      <div className="grid">
        {movies.map((card) => (
          <MovieCard key={card.movie.id} card={card} />
        ))}
      </div>
    </section>
  )
